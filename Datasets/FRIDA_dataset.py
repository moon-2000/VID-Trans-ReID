...
class FRIDA(object):
    ...
    #                 tracklet = ((img_path,), pid, self.cameras.index(camera))  # Update tracklet structure
    #                 if pid in selected_persons_train:
    #                     tracklets_train.append(tracklet)
    #                     num_imgs_per_tracklet_train.append(len(tracklet[0]))  # Access the tuple of image paths
    #                 elif pid in selected_persons_test:
    #                     tracklets_test.append(tracklet)
    #                     num_imgs_per_tracklet_test.append(len(tracklet[0]))  # Access the tuple of image paths
        ...
        for segment in dirnames:
            for camera in self.cameras:
                ...
                for person_info in data:
                    ...
                    # Create a list to store image paths for the current tracklet
                    tracklet_images = []
                    ...
                    tracklet = (tuple(tracklet_images), pid, self.cameras.index(camera))
                    
                    if pid in selected_persons_train:
                        tracklets_train.append(tracklet)
                        num_imgs_per_tracklet_train.append(len(tracklet_images))  # Tracklet contains all images for the person
                    elif pid in selected_persons_test:
                        tracklets_test.append(tracklet)
                        num_imgs_per_tracklet_test.append(len(tracklet_images))  # Tracklet contains all images for the person

        # Filter out tracklets with fewer images than the specified minimum sequence length
        tracklets_train = [tracklet for tracklet in tracklets_train if len(tracklet[0]) >= min_seq_len]
        tracklets_test = [tracklet for tracklet in tracklets_test if len(tracklet[0]) >= min_seq_len]
